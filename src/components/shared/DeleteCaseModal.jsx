...
import { deleteArchivedCase, deleteSingleCase } from "../../services/cases";
import { message } from "antd";

export const DeleteCaseModal = ({ selectedCaseId, caseDeleteModalVisible, setCaseDeleteModalVisible, onDeleteSuccess, fromArchivedPage = false }) => {
  const [caseDeleted, setCaseDeleted] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleDeleteCase = () => {
    let functionToTrigger = deleteSingleCase;
    if (fromArchivedPage) {
      functionToTrigger = deleteArchivedCase;
    }
    setIsSubmitting(true);
    functionToTrigger(selectedCaseId)
      .then((_response) => {
      ...
      .catch((err) => {
        message.error(err?.message || "Error delete case");
        setIsSubmitting(false);
      });
  };
  ...
  return (
    <>
      ...
      <ActionModal
        open={caseDeleted}
        onCancel={() => setCaseDeleted(false)}
        onConfirm={() => { }}
        title="Case Deleted"
        content={`Congratulations! You has successfully deleted the case.`}
        showButtons={false}
        icon={
